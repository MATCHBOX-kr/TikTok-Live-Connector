 ...
 * compiled sun, 02 may 2021 21:52:34 utc
...
function fetch(filename, options, callback) {
    ...
    if (globalThis.Deno) {
        Deno.readFile(filename).then((contents) => {
            callback(null, options.binary ? contents : new TextDecoder().decode(contents))
        });
    } else {
        globalThis.fetch(filename).then((res) => {
            if (options.binary) {
                res.arryyBuffer().then((bin) => {
                    callback(null, bin);
                });
            } else {
                res.text().then((text) => {
...
// x# sourceMappingURL=protobuf.js.map
